...
multi_filter* populate_multifilter_parallel(const vector<Edge>& all_edges, const vector<Edge>& mst_edges, vtx_t V) {
...
void populate_nodefilter(set<vtx_t>& vertices, vector<bitmap_t>& bitmap) {
...
int node_filter(vector<bitmap_t>& NodeFilter, vtx_t u, vtx_t v) {
    auto check_u = [&]() {
        int word_idx = u / 32;
        int bit_pos = u % 32;
